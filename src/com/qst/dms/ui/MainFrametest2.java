...
public class MainFrametest2 extends JFrame {
	...
	private JMenuItem miGather, miSave, miSend,
			miShow, miExit, miCheck, miAbout, LogMatch, TransMatch;
	...
	private JComboBox<String> transStatus;
	...
	public MainFrametest2() {
		...
		transStatus = new JComboBox<String>();
		// 初始化菜单
		initMenu();
		// 初始化工具栏
		initToolBar();
	...
	private void initMenu() {
		...
		//menuMatch.add(lblNewLabel);
		
		LogMatch = new JMenuItem("匹配日志信息");
		LogMatch.addActionListener(new MatchLogListener());
		menuMatch.add(LogMatch);
		
		
		TransMatch = new JMenuItem("匹配物流信息");
		TransMatch.addActionListener(new MatchTransListener());
		menuMatch.add(TransMatch);
	...
	private void initToolBar() {
		...
		btnMatchTran = new JButton("匹配物流数据", null);
	...
	private void initTransGatherGUI() {
		...
		transStatus = new JComboBox();
		transStatus.setModel(new DefaultComboBoxModel(new String[] {"发货中", "送货中", "已签收"}));
		pTranStatus.add(transStatus);
	...
	private class GatherTransListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 设置物流类型
			//int transportType2 = cmbTanStatus.getSelectedIndex() + 1;
			int transportType = transStatus.getSelectedIndex() + 1;
			// 将数据包装成物流对象
	...
	private class MatchLogListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(matchedLogs.size()!=0)
				...
				JOptionPane.showMessageDialog(null, "匹配失败，没有可匹配的信息！", "错误",
	...
	private class SaveDataListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			//若没有保存成功，则弹出相应的告警提示框
			transportService.saveMatchTransportToDB(matchedTrans);	//待改进
			// 保存匹配的物流信息
